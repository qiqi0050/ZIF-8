...
ZIF-8是MOFs材料的一个子类，优点是它拥有含量较多的N原子、较高的比表面积、稳定的化学性能，是用来制备N掺杂多孔C材料的前驱体，因而被广泛的应用在氧还原反应（ORR）等领域。衍生自金属--有机骨架（MOFs）的氮和金属共掺杂碳的固有颗粒性质阻碍了其在氧还原反应（ORR）中的实际应用。本文中，我们开发了一种新型的含N，Zn的基于MOFs的分层碳纤维作为ORR催化剂。通过将Zn沸石咪唑化物框架（Zn-ZIF）与电纺聚丙烯腈纤维结合，合成产物ZIF-8—PAN纤维，并系统地研究了温度和前体对最终样品的ORR性能的影响。
\end{abstract}
